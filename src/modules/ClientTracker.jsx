...
import { getClients, addClient, updateClient, deleteClient, getSettings } from '../utils/storage.js'
  ...
  return (
    <div className="animate-fadeIn">
      <div style={g2}>
        ...
        <Field label="Contact Person"><Input value={f.contactName} onChange={v => set('contactName', v)} placeholder="e.g. Kojo Mensah" /></Field>
        <Field label="Phone"><Input value={f.phone} onChange={v => set('phone', v)} placeholder="e.g. +233 XX XXX XXXX" /></Field>
        <Field label="Email"><Input value={f.email} onChange={v => set('email', v)} placeholder="e.g. kojo@example.com" type="email" /></Field>
        <Field label="Industry"><Input value={f.industry} onChange={v => set('industry', v)} placeholder="e.g. Marine Logistics" /></Field>
        ...
        <Field label="Monthly Value"><Input value={f.monthlyValue} onChange={v => set('monthlyValue', v)} placeholder="e.g. 800" type="number" /></Field>
        <Field label="GBP Audit Score (/ 10)"><Input value={f.auditScore} onChange={v => set('auditScore', v)} placeholder="e.g. 3" type="number" /></Field>
...
function ClientCard({ client, onEdit, onDelete, cur }) {
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: T.textLight, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            ...
            {client.monthlyValue && (
              <span style={{ color: T.success, fontWeight: 700 }}>{cur}{client.monthlyValue}/mo</span>
            )}
      ...
      {expanded && (
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px solid ${T.grayBorder}` }} className="animate-fadeIn">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
            ...
            {client.website && <div><span style={{ fontSize: 11, color: T.textLight, fontWeight: 600 }}>WEBSITE</span><div style={{ fontSize: 13 }}><a href={client.website} target="_blank" rel="noreferrer" style={{ color: T.blue }}>Visit ↗</a></div></div>}
...
export default function ClientTracker() {
  ...
  useEffect(() => {
    loadAll()
    window.addEventListener('fds-settings-updated', loadAll)
    return () => window.removeEventListener('fds-settings-updated', loadAll)
  }, [])
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 22 }}>
        {[
          ...
          { label: 'Monthly Revenue', value: `${cur}${totalMRR}`, color: T.gold, bg: T.goldLight },
      ...
      {/* LIST */}
      {filtered.length === 0
        ? <EmptyState icon="📋" title={clients.length === 0 ? 'No clients yet' : 'No results'} subtitle={clients.length === 0 ? 'Add your first client above.' : 'Adjust your search or filter.'} />
        : filtered.map(c => <ClientCard key={c.id} client={c} onEdit={handleEdit} onDelete={handleDelete} cur={cur} />)
      }
    </div>
  )
}
